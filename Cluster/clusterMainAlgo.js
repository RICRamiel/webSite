    ...
    while (iterate) {
        ...
        for (let dataIndex = 0; dataIndex < data.length; dataIndex += 1) {
            for (let clusterIndex = 0; clusterIndex < k; clusterIndex += 1) {
                distances[dataIndex][clusterIndex] = euclideanDistance([clusterCenters[clusterIndex]], [data[dataIndex]],);
            }
            const closestClusterIdx = distances[dataIndex].indexOf(Math.min(...distances[dataIndex]),);
        ...
        for (let clusterIndex = 0; clusterIndex < k; clusterIndex += 1) {
            ...
            for (let dimensionIndex = 0; dimensionIndex < dataDim; dimensionIndex += 1) {
                clusterCenters[clusterIndex][dimensionIndex] = parseFloat(Number(clusterCenters[clusterIndex][dimensionIndex] / clusterSize,).toFixed(2));
            }
        }
    }
    return classes;
